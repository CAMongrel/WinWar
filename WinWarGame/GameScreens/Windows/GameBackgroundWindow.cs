...
namespace WinWarGame.GameScreens.Windows
{
    class GameBackgroundWindow : UIWindow
    {
        ...
        private void InitUI()
        {
            ...
            EntityControl = new UIEntityControl(LevelGameScreen.Game.InterfaceRace);
            MinimapControl = new UIMinimapControl(MapControl);

            LoadUIImage(ref leftSidebarTop, "Sidebar Left Minimap Black (" + LevelGameScreen.Game.InterfaceRace + ")");
            LoadUIImage(ref leftSidebar, "Sidebar Left (" + LevelGameScreen.Game.InterfaceRace + ")");
            leftSidebar.Y = leftSidebarTop.Height;

            LoadUIImage(ref topBar, "Topbar (" + LevelGameScreen.Game.InterfaceRace + ")");
            LoadUIImage(ref bottomBar, "Lower Bar (" + LevelGameScreen.Game.InterfaceRace + ")");

            topBar.X = leftSidebarTop.Width;
            bottomBar.X = leftSidebar.Width;
            bottomBar.Y = 200 - bottomBar.Height;

            LoadUIImage(ref rightBar, "Sidebar Right (" + LevelGameScreen.Game.InterfaceRace + ")");
            ...
            menuButton = new UIButton("", WarFile.KnowledgeBase.IndexByName("Menu Button"),
                WarFile.KnowledgeBase.IndexByName("Menu Button (Pressed)"));
        ...
        private void menuButton_OnMouseUpInside(Vector2 position)
        {
            IngameMenuWindow menu = new IngameMenuWindow(levelGameScreenOwner.InterfaceRace);
        }
