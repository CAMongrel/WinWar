...
namespace WinWarGame.Data.Game
{
    ...
    /// </summary>
    class Map : IScriptObject, IScriptMap
    {
        ...
        internal int StartCameraX => levelInfo?.StartCameraX ?? 0;
        internal int StartCameraY => levelInfo?.StartCameraY ?? 0;
        ...
        private readonly MapDiscover[] mapDiscoverState;
        ...
        /// Create map
        /// </summary>
        internal Map(string setName,
            LevelInfoResource setLevelInfo,
        ...
        private void AddPlayers()
        {
            ...
            for (int i = 0; i < levelInfo.PlayerInfos.Length; i++)
            {
                ...
                pl.SetGold(levelInfo.PlayerInfos[i].StartGold);
                pl.SetLumber(levelInfo.PlayerInfos[i].StartLumber);
        ...
        internal void Start()
        {
            ...
            levelScript?.OnLevelInitialized(this);
        }
        ...
        internal void Update(GameTime gameTime)
        {
            ...
            levelScript?.UpdateLevel(gameTime.ElapsedGameTime.TotalSeconds);
        ...
        private void PopulateInitialEntities()
        {
            ...
            for (int i = 0; i < levelInfo.StartObjects.Length; i++)
            {
                ...
                {
                    Log.Write(LogType.Generic, LogSeverity.Status,
                        "Failed to place entity of type '" + lo.Type + "' at " + lo.X + "," + lo.Y + ".");
                }
            }
        }
        ...
        internal bool CreateEntity(int x, int y, LevelObjectType entityType, BasePlayer owner)
        {
            ...
            Log.Write(LogType.Generic, LogSeverity.Debug,
                "levelPassable at [" + x + "," + y + "]: " + levelPassable.passableData[x, y]);
            if (Pathfinder[x, y] != 0)
            {
                return false;
            }
        ...
        internal void RemoveEntity(Entity ent)
        {
            ...
            {
                entity.HateList.RemoveEntity(ent);
            }
            ...
            if (selectionDidChange && OnSelectedEntitiesChanged != null)
            {
        ...
        internal void Render(int setX, int setY, int setWidth, int setHeight, float tileOffsetX, float tileOffsetY)
        {
            ...
            for (int y = 0; y < tilesToDrawY; y++)
            {
                for (int x = 0; x < tilesToDrawX; x++)
                {
                    ...
                    tileSet.DrawTile(index, setX + x * TileWidth - innerTileOffsetX,
                        setY + y * TileHeight - innerTileOffsetY, 1.0f);
                    ...
                    if (DebugOptions.ShowBlockedTiles)
                    {
                        ...
                        if (isBlocked)
                        {
                            ...
                            WWTexture.SingleWhite.RenderOnScreen(setX + x * TileWidth - innerTileOffsetX,
                                setY + y * TileHeight - innerTileOffsetY,
            ...
            for (int i = 0; i < Roads.Count; i++)
            {
                ...
                if (isVisible)
                    tileSet.DrawRoadTile(road.Config, setX + x * TileWidth - innerTileOffsetX,
                        setY + y * TileHeight - innerTileOffsetY, 1.0f);
            }
        ...
        internal Color[] GetMinimap()
        {
            ...
            // This is not efficient (could also do this conditionally in each of the loops above),
